# -*- coding: utf-8 -*-
from __future__ import division, print_function
from ogs5py import OGS, GLIext

model = OGS(
    task_root='gian_quad_root',
    task_id='gian_quad',
    output_dir='out',
)
model.gli.read_file('gian_quad.gli')
model.ic.add_block(
    main_key='INITIAL_CONDITION',
    PCS_TYPE='OVERLAND_FLOW',
    PRIMARY_VARIABLE='HEAD',
    GEO_TYPE='DOMAIN',
    DIS_TYPE=['CONSTANT', 0.0001],
)
model.mfp.add_block(
    main_key='FLUID_PROPERTIES',
    FLUID_TYPE='LIQUID',
    PCS_TYPE='HEAD',
    DENSITY=[1, 1000.0],
    VISCOSITY=[1, 0.001],
)
model.mmp.add_block(
    main_key='MEDIUM_PROPERTIES',
    GEO_TYPE=['SURFACE:', 'SURFACE0'],
    SURFACE_FRICTION=[66.67, 0.5, 0.67],
    RILL=[0.0, 0.0],
)
model.mmp.add_block(
    main_key='MEDIUM_PROPERTIES',
    GEO_TYPE=['SURFACE:', 'SURFACE0'],
    SURFACE_FRICTION=[6.67, 0.5, 0.67],
    RILL=[0.0, 0.0],
)
model.msh.read_file('gian_quad.msh')
model.num.add_block(
    main_key='NUMERICS',
    PCS_TYPE='OVERLAND_FLOW',
    LINEAR_SOLVER=[2, 5, 1e-10, 1000, 1.0, 100, 4],
    NON_LINEAR_SOLVER=['NEWTON', 1e-05, 1e-08, 100, 0.0],
    ELE_GAUSS_POINTS=2,
)
model.out.add_block(
    main_key='OUTPUT',
    NOD_VALUES=[
        ['HEAD'],
        ['WDEPTH'],
        ['FLUX'],
    ],
    PCS_TYPE='OVERLAND_FLOW',
    GEO_TYPE='DOMAIN',
    DAT_TYPE='TECPLOT',
    TIM_TYPE=['STEPS', 10],
)
model.out.add_block(
    main_key='OUTPUT',
    NOD_VALUES=[
        ['WDEPTH'],
        ['HEAD'],
        ['FLUX'],
    ],
    PCS_TYPE='OVERLAND_FLOW',
    GEO_TYPE=['POINT', 'POINT4'],
    DAT_TYPE='TECPLOT',
    TIM_TYPE=['STEPS', 10],
)
model.out.add_block(
    main_key='OUTPUT',
    NOD_VALUES=[
        ['WDEPTH'],
        ['HEAD'],
        ['FLUX'],
    ],
    PCS_TYPE='OVERLAND_FLOW',
    GEO_TYPE=['POINT', 'POINT5'],
    DAT_TYPE='TECPLOT',
    TIM_TYPE=['STEPS', 10],
)
model.pcs.add_block(
    main_key='PROCESS',
    PCS_TYPE='OVERLAND_FLOW',
    NUM_TYPE='NEW',
)
model.rfd.read_file('gian_quad.rfd')
model.st.add_block(
    main_key='SOURCE_TERM',
    PCS_TYPE='OVERLAND_FLOW',
    PRIMARY_VARIABLE='HEAD',
    GEO_TYPE=['SURFACE', 'SFCTOP'],
    DIS_TYPE=['CONSTANT_NEUMANN', 1.0],
    TIM_TYPE=['CURVE', 1],
)
model.st.add_block(
    main_key='SOURCE_TERM',
    PCS_TYPE='OVERLAND_FLOW',
    PRIMARY_VARIABLE='HEAD',
    GEO_TYPE=['POLYLINE', 'CRITICALDEPTH'],
    DIS_TYPE=[
        ['CRITICALDEPTH', 1],
        [0.0],
    ],
)
model.tim.add_block(
    main_key='TIME_STEPPING',
    PCS_TYPE='OVERLAND_FLOW',
    TIME_STEPS=[
        [1, 5.0],
        [1, 5.0],
        [1, 10.0],
        [1, 10.0],
        [1, 20.0],
        [1, 20.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 37.5],
        [1, 37.5],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 50.0],
        [1, 25.0],
        [1, 25.0],
    ],
    TIME_END=10795,
    TIME_START=0.0,
    TIME_CONTROL=[],
)
gli_ext_file = GLIext(
    file_name='gian_quad_sfc',
    file_ext='.tin',
)
gli_ext_file.read_file('gian_quad_sfc.tin')
model.add_gli_ext(gli_ext_file)
model.write_input()
model.run_model()
