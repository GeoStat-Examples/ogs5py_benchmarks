...
model = OGS(
    task_root='3d_grain_root',
    task_id='3d_grain',
    output_dir='out',
)
model.msh.read_file('3d_grain.msh')
model.gli.read_file('3d_grain.gli')
model.pcs.add_block(
    main_key='PROCESS',
...
model.mmp.add_block(
    ...
    STORAGE=[1, 0.0],
    TORTUOSITY=[1, 1.0],
    PERMEABILITY_TENSOR=['ISOTROPIC', 0.0001],
...
model.pct.read_file('3d_grain.pct')
model.num.add_block(
    main_key='NUMERICS',
    PCS_TYPE='GROUNDWATER_FLOW',
...
model.num.add_block(
    ...
    PCS_TYPE='FLUID_MOMENTUM',
    ELE_GAUSS_POINTS=2,
    LINEAR_SOLVER=[2, 5, 1e-16, 1000, 1.0, 1, 2],
)
model.tim.add_block(
    main_key='TIME_STEPPING',
    PCS_TYPE='GROUNDWATER_FLOW',
    TIME_STEPS=[20, 100000.0],
    TIME_END=1e+99,
    TIME_START=0.0,
)
model.tim.add_block(
